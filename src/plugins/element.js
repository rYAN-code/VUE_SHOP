...
import Element, {
  ...
  Table, TableColumn, Pagination, Dialog, MessageBox, Tag, Tree, Select, Cascader
} from 'element-ui'
...
Vue.use(Element).use(Button).use(Form).use(FormItem).use(Input).use(Container).use(Header).use(Aside).use(Main)
  ...
  .use(Table).use(TableColumn).use(Pagination).use(Dialog).use(Tag).use(Tree).use(Select).use(Cascader)
// 原型挂载
